...
setupRemote() {
    ...
    scp -i ${SSH_KEY_PATH} -r ./convenience-scripts/ ${REMOTE_USER}@${REMOTE_IP}:/tmp
    runRemoteCommand 'sudo cp -a /tmp/convenience-scripts/. /home/ubuntu && rm -rf /tmp/convenience-scripts'
    echo "------------------------------------------------------"
