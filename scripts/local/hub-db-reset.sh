...
docker run -d -e POSTGRES_PASSWORD=postgres -e POSTGRES_USERNAME=postgres --name ai-dev-postgres -p 5433:5432 postgres:12
...
flyway \
-url=jdbc:postgresql://localhost:5433/postgres?sslmode=disable \
-user=postgres \
-password=postgres \
-schemas=hub \
